...
class ELOAnnotate:
    ...
    def __init__(self, data_path : Path, out_path : Path, auto_save_every = 10):
    ...
    def _pick_images(self):
        ...
        if self.total_games == 0:
            probs = {name: 1 for name in self.annotations if not self.annotations[name][3]}
        else:
            probs = {name: math.exp(-games) \
                            for name, (_, _, games, discard)  in self.annotations.items() \
                            if not discard}
        ...
        names = np.random.choice(names, 101, p=probs, replace=False)
        
        # pick image two as the image with the closest ELO rating to image one
        name1 = names[0]
        name2 = min(names[1:], key=lambda name: abs(self.annotations[name][1] - self.annotations[name1][1]))
    ...
    def _update_display(self):
        ...
        self.fig.text(0.7, 0.98, f"'1':pick left, '2':pick right\n \
                                    't':discard left, 'y':discard right\n \
                                    'q':quit and save",
    ...
    def _on_key(self, event):
        ...
        match event.key:
            ...
            case 't':
                self._discard(self.tmp_name1)
            case 'y':
                self._discard(self.tmp_name2)
            case 'q':
...
if __name__ == "__main__":
    ...
    annotater = ELOAnnotate(data_folder, output_folder / output_csv)
    annotater.annotate()
